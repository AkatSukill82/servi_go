...
Deno.serve(async (req) => {
  try {
    ...
    const subDetailsHtml = [
      `Plan : <strong>${isAnnual ? 'Annuel — 100 €/an' : 'Mensuel — 10 €/mois'}</strong>`,
      data.renewal_date ? `Prochain renouvellement : <strong>${data.renewal_date}</strong>` : null,
      data.trial_ends_date ? `Fin de la période d'essai : <strong>${data.trial_ends_date}</strong>` : null,
    ...
    const html = buildEmailHtml({
      ...
      body: `
        ...
        ${infoBlock('Votre abonnement', subDetailsHtml)}
        <p style="margin-bottom:8px;font-weight:600;">Pour recevoir vos premières missions :</p>
        <ol style="margin:0;padding-left:20px;font-family:'Bricolage Grotesque','Helvetica Neue',Arial,sans-serif;font-size:14px;color:#444444;line-height:2;">
