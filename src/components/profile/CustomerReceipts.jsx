...
export default function CustomerReceipts({ user }) {
  const { data: requests = [], isLoading } = useQuery({
    queryKey: ['customerRequests', user?.email],
    queryFn: () => base44.entities.ServiceRequest.filter({ customer_email: user.email }, '-created_date', 50),
    enabled: !!user?.email,
  });
  ...
  // Associer les invoices aux requests
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-card rounded-xl p-4 border border-border">
          <p className="text-xs text-muted-foreground font-medium mb-1">Total dépensé</p>
              ...
              >
                ...
                <div className="flex items-center justify-between mt-3">
                  <div className="text-xs text-muted-foreground">
                    ...
                    {req.scheduled_date && (
                      <span className="ml-2">· prévu le {format(new Date(req.scheduled_date), 'dd MMM', { locale: fr })}</span>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    <p className="font-bold text-base">{(req.total_price || 0).toFixed(2)} €</p>
                    {invoice && (
                      <button
