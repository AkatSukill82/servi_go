...
export default function CustomerContracts({ user }) {
  const [selectedContract, setSelectedContract] = useState(null);

  const { data: contracts = [], isLoading } = useQuery({
    queryKey: ['customerContracts', user?.email],
    queryFn: () => base44.entities.MissionContract.filter({ customer_email: user.email }, '-created_date', 50),
    enabled: !!user?.email,
  });
    ...
    <>
      <div className="space-y-3">
        {contracts.map((contract, i) => {
          ...
          const customerSigned = !!contract.signature_customer;
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">{contract.category_name} · {contract.professional_name}</p>
                  ...
                  <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full shrink-0 ${
                    ...
                    customerSigned ? 'bg-blue-100 text-blue-700' :
                    'bg-yellow-100 text-yellow-700'
                  }`}>
                    {isCompleted ? '✓ Terminé' : bothSigned ? '✓ Signé' : customerSigned ? 'En attente pro' : 'À signer'}
                  </span>
                </div>
                ...
                >
                  ...
                  {!customerSigned ? 'Voir & Signer le contrat' : 'Voir le contrat'}
