        ...
        <div className="px-5 pt-6 pb-4">
          <div className="flex items-start justify-between mb-5">
            <div>
              ...
              <h1 className="text-[26px] font-bold text-gray-900 mt-0.5 tracking-tight leading-tight">
                {firstName ? `${greeting}, ${firstName}` : 'Trouvez un expert'}
              </h1>
            </div>
            {/* Notification bell */}
            <button
              className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center mt-1 shrink-0"
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full h-[52px] pl-11 pr-4 rounded-2xl bg-gray-50 text-gray-900 text-[15px] placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#6C5CE7]/30 transition-all"
              style={{ fontSize: 15 }}
            />
            {searchQuery && (
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-[13px] font-semibold text-gray-900">En attente — {unfinishedRequest.category_name}</p>
                  <p className="text-[12px] text-gray-500 mt-0.5">Recherche d'un professionnel…</p>
                </div>
              ...
              {!confirmCancel ? (
                ...
                <div className="mt-2 flex gap-2">
                  <button onClick={() => setConfirmCancel(false)} className="flex-1 text-[12px] font-medium border border-gray-200 rounded-xl py-2 bg-white">Non</button>
                  <button
        ...
        <div className="px-5 space-y-7 pb-6">
          ...
          <section>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-[17px] font-bold text-gray-900">Nos services</h2>
              {searchQuery && (
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center py-10 text-center">
                ...
                <p className="text-[14px] font-medium text-gray-500">Aucun service trouvé</p>
              </div>
            ) : (
          ...
          {nearbyPros.length > 0 && (
            <section>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-[17px] font-bold text-gray-900">Pros les mieux notés</h2>
          ...
          {recentReviews.length > 0 && (
            <section>
              <h2 className="text-[17px] font-bold text-gray-900 mb-4">Ce que disent nos clients</h2>
                  ...
                  >
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] font-semibold text-gray-900 truncate">{review.customer_name || 'Client'}</p>
                        <p className="text-[11px] text-gray-400">{review.category_name}</p>
                      </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={label} className="bg-gray-50 rounded-2xl p-3 flex flex-col items-center gap-1.5 text-center">
...
function ProCard({ pro, index, onPress }) {
  return (
    <motion.button
      ...
      onClick={onPress}
      className="flex-none w-[148px] snap-start bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm active:scale-[0.97] transition-transform text-left"
    >
      {/* Avatar */}
      <div className="w-full h-[110px] bg-gray-100 flex items-center justify-center overflow-hidden">
        {pro.photo_url
          ? <img src={pro.photo_url} alt="" className="w-full h-full object-cover" />
          : <span className="text-3xl font-bold text-gray-300">{(pro.full_name || '?')[0].toUpperCase()}</span>
        }
      </div>
      <div className="p-3">
        <p className="text-[13px] font-bold text-gray-900 truncate">{pro.full_name || pro.name || 'Pro'}</p>
        <p className="text-[11px] text-gray-400 truncate">{pro.category_name}</p>
        {pro.rating && (
          <div className="flex items-center gap-1 mt-1.5">
            <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
            <span className="text-[12px] font-semibold text-gray-700">{pro.rating}</span>
            {pro.reviews_count && <span className="text-[11px] text-gray-400">({pro.reviews_count})</span>}
          </div>
        )}
      </div>
    </motion.button>
